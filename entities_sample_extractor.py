...
def annotate_case(case: extract_node_values.Case, entity_typename_builder: build_entity_dictionary.EntityTypename, nlp: Language) -> List[Doc]:
    """
    Annotate one case.
    ...
    annotate_case.complete_case_annotations(spacy_docs, spans)

    return spacy_docs


def save_doc(case: extract_node_values.Case, spacy_docs: List[Doc], directory: str) -> None:
    """
...
def annotate(model_dir_path: str, files_dir_path: List[str], out_dir_path: str) -> None:
    ...
    nlp = model_factory.get_empty_model(load_labels_for_training=False)
    ...
    entity_typename_builder = build_entity_dictionary.EntityTypename()

    logging.info("Loading cases…")

    cases: List[extract_node_values.Case] = list()
    for path in files_dir_path:
        if path.endswith(".xml"):
            case: extract_node_values.Case = extract_node_values.get_paragraph_from_file(path=path,
                                                 keep_paragraph_without_annotation=True)
            cases.append(case)
        elif path.endswith(".txt"):
            with open(path) as f:
                lines = f.readlines()
                case: extract_node_values.Case = list()
                for line in lines:
                    clean_text = line.strip()
                    if len(clean_text) > 1:
                        basename = os.path.basename(path)
                        basename = basename.split(".")[0]
                        case.append(extract_node_values.Paragraph(basename, clean_text, list(), list()))
                cases.append(case)
        else:
