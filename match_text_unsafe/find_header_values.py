...
from common_xml_parser_function import read_xml
...
def parse_xml_header(path: str) -> Dict[str, Dict[str, list]]:
    ...
    nodes = tree.xpath('//annonce//annonceInfo//adID|//evenements//evenement//personnes//personne//identite//nom|'
                       '//evenements//personnes//personne//identite//etablissements//etablissement//adresse//codePostal|'
                       '//evenements//evenement//dateDecision')
    ...
    for node in nodes:
        ...
        if node.tag == "adID":
            ...
            # added by piter
            current_case['company_name'] = company_name
            current_case['date'] = date
            current_case['cost'] = cost
            headers_content[node.text] = current_case
        ...
        # added by piter
        if node.tag == "nom":
            company_name.append(node.text)
        if node.tag == "codePostal":
            cost.append(node.text)
